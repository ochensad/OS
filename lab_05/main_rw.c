...
{
  if (semop(semid, start_write_sbuf, 4) == -1)
    return -1;
...
void writer(int semid, int shmid, int perms)
{
...
void reader(int semid, int shmid, int perms)
{
  int *number = shmat(shmid, 0, perms);
  if (number == (int *)-1)
  {
