...
int consumer(int semid, char **caddr, int shmid, int perms) 
{
...
int producer(int semid, char **paddr, char *alpha, int shmid, int perms)
{
  void *buf = shmat(shmid, 0, perms);
  if (buf == (char *)-1)
  {
  ...
  *caddr = *paddr;
  ...
  for (int i = 0; i < PROD_CNT; i++)
  {
    if ((chpid[i] = fork()) == -1)
      log_exit("cant fork");
    if (chpid[i] == 0) 
      producer(semid, paddr, alpha, shmid, perms);
    // else
    // {
    //   int status;
  ...
  for (int i = PROD_CNT; i < CONS_CNT + PROD_CNT; i++)
  {
    if ((chpid[i] = fork()) == -1)
      log_exit("cant fork");
    if (chpid[i] == 0)
      consumer(semid, caddr, shmid, perms);
    // else
    // {
    //   int status;
    //   if (waitpid(chpid[i], &status, WUNTRACED) == -1)
